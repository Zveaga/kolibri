      ...
      <template #default>
        <div>
          <KTextbox
            ...
            :invalid="Boolean(isClassNameInvalid)"
            :invalidText="isClassNameInvalid"
          />
        ...
        <div class="bottom-nav">
          ...
          <div>
            ...
            <KButton
              :text="copyClasslabel$()"
              :primary="true"
              :disabled="Boolean(isClassNameInvalid) || submitting"
...
<script>
  ...
  export default {
    ...
    computed: {
      isClassNameInvalidBool() {
        const normalize = str => str.replace(/\s+/g, '').toUpperCase();

        const normalizedCopied = normalize(this.copiedClassName);

        return (
          this.classroom.some(row => normalize(row[0]) === normalizedCopied) && !this.submitting
        );
      },
      isClassNameInvalid() {
        return this.isClassNameInvalidBool ? this.classNameAlreadyExists$() : '';
      },
      userRoleBadgeStyle() {
        return {
